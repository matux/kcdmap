const version = 1.3,
    ...
    tilesUrl = "./map/{z}_{x}_{y}.jpg";
let maxNativeZoom = 5,
    mapMinZoom = 1,
    mapMaxZoom = 5,
    mapSize = 8192,
    tileSize = 256,
    mapScale = mapSize / tileSize,
    mapOffset = mapSize / mapScale / 2,
    halfTile = tileSize / 2,
    mapBounds = 4096;
...
function gridfix() {
    ...
    L.GridLayer.include({
        _initTile: function(t) {
            ...
            (t.style.width = a.x + 0.5 + "px"), (t.style.height = a.y + 0.5 + "px");
        },
    });
}
...
for (let e = 0; e < markers.length; e++) {
    ...
    let a = markers[e].coords[1].toFixed(0),
        i = markers[e].coords[0].toFixed(0),
        r = markers[e].coords[1].toFixed(0),
        o = markers[e].coords[0].toFixed(0),
        n = url + "?marker=" + i + "," + a;
    n = encodeURI(n);
    let s = L.marker([a, i], {
            icon: getIcon(e),
...
function toggle(e, t) {
    e.checked ?
        ...
        ($("#allmarkers").prop("checked", !1), map.removeLayer(layerGroups[t]));
}
...
$(allmarkers).click(function() {
        ...
        } else 0 == $(this).prop("checked") && localStorage.setItem("activemarkers", "[]");
    }),
    $(".markers-list input").each(function() {
...
function getAObj(t, a) {
    for (e in t)
        if (t[e].name == a) return t[e].value;
    return 0;
}
markers.forEach(function(e) {
        ...
        locatedGroup.addLayer(t), (e.marker_id = locatedGroup.getLayerId(t));
    }),
    markers.forEach(function(e) {
...
for (let e in mapMarkers) {
    let t = mapMarkers[e].icon,
        a = mapMarkers[e].width,
        i = mapMarkers[e].height;
    ...
    if (
        ...
        for (let a = 0; a < t.length; a++) {
            let i = t[a].coords.x,
                r = t[a].coords.y,
                o = t[a].name,
                n = (t[a].icon, t[a].iconvalue),
                s = t[a].icon.options.iconUrl,
                l = t[a].title,
                c = t[a].desc,
                d =
                url +
    ...
    popup.setLatLng(i).setContent(a).openOn(map),
        $("#addmark").submit(function(e) {
            ...
            let t = $(this).serializeArray(),
                a = Math.round(getAObj(t, "mlat")),
                i = Math.round(getAObj(t, "mlon"));
            t.push({
                name: "lat",
                value: a
            }), t.push({
        ...
        $(document).on("click", ".edit-marker", function() {
            for (
                ...
                ($(this)
                    ...
                    .css(
                        "background-image",
                        "url(" + storageMarkers[i].icon.options.iconUrl + ")"
                    ),
                    $(this)
                    .parent()
                    .find("#select_icon")
            ...
            for (
                ...
                if (
                    ...
                    let e = $(this).parent().find("select[name=icon]").val(),
                        r = $(this).parent().find("#editedtitle").val(),
                        n = $(this).parent().find("#editeddesc").val(),
                        s =
                        url +
                        "?m=" +
                        a.lng +
...
function onPopupOpenShared() {
    ...
    let n = sharedMarker.split(",")[1],
        s = sharedMarker.split(",")[0],
        l = markerIconTypes[i].options.iconUrl;
    $(document).off("click", ".remove-marker"),
        $(document).on("click", ".remove-marker", function() {
            $(this).addClass("hide"),
                $(this).next("#remove-dialog").removeClass("hide"),
        ...
        $(document).on("click", ".save-marker", function() {
            ...
            let i = $(this).parent().find("select[name=icon]").val(),
                r = $(this).parent().find("#editedtitle").val(),
                o = $(this).parent().find("#editeddesc").val(),
                l =
                '<div class="popcontent"><p class="mtitle">' +
                r +
                '</p><p class="mdesc">' +
...
$(document).on("click", ".clearls", function() {
    ...
    $(".backupls").on("click", function(e) {
        let t = {},
            a = localStorage.mapUserMarkers,
            i = localStorage.langactive;
        (t.markers = a), (t.langactive = i);
        let r = JSON.stringify(t),
            o = "data:text/javascript;charset=utf-8;base64," + btoa(r),
